...
class LEDResistorCalculator(QWidget):
    ...
    def calculate_parallel(self):
        try:
            ...
            self.p_calc_res_label.setText(f"Calculated Resistance: {res_val:.2f} Ω")
            self.p_std_res_label.setText(f"Suggested Standard (E24): {std_val} Ω")
            self.p_long_life_calc_label.setText(f"Long Life Calculated: {long_res_val:.2f} Ω")
            self.p_long_life_std_label.setText(f"Long Life Standard (E24): {long_std_val} Ω")
        except ValueError:
            QMessageBox.critical(self, "Input Error", "Please enter valid numerical values.")
